    ...
    <template x-teleport="body">
        <div x-show="show" 
            ...
            <div x-show="show" 
                    ...
                    class="relative w-full max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
                ...
                <div class="flex flex-1 overflow-hidden">
                    {{-- Sidebar --}}
                    <div class="w-1/3 min-w-[200px] bg-gray-50/50 border-r border-gray-100 p-4 space-y-2 overflow-y-auto">
                            @foreach(['sort' => 'Sort', 'category' => 'Category', 'date' => 'Date', 'price' => 'Price'] as $tab => $label)
                            <button 
                                wire:click="setActiveTab('{{ $tab }}')"
                                @class([
                                    'w-full text-left px-4 py-3 rounded-xl font-bold text-sm transition-all flex items-center justify-between group',
                    ...
                    <div class="flex-1 p-8 overflow-y-auto">
                        ...
                        @if(!empty($selectedCategories) || $dateFilter !== 'all' || $sort !== 'most_popular' || $minPrice || $maxPrice)
                            <div class="mb-8">
                                ...
                                @if($dateFilter === 'other')
                                    <div class="p-6 bg-gray-50 rounded-2xl border border-gray-200 space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
                                        <div class="grid grid-cols-2 gap-4">
                                            <div class="space-y-2">
                                                <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">Start</label>
                                                <input type="date" wire:model.live="startDate" class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-red-500 focus:border-red-500 text-gray-700 font-medium">
                                            </div>
                                            <div class="space-y-2">
                                                <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">End</label>
                                                <input type="date" wire:model.live="endDate" class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-red-500 focus:border-red-500 text-gray-700 font-medium">
                                            </div>
                                        </div>
                                    </div>
                                @endif
                ...
                {{-- Footer --}}
                <div class="p-6 border-t border-gray-100 flex items-center justify-between bg-white z-10 relative">
                    <button wire:click="resetFilters" class="text-gray-500 font-bold hover:text-red-500 transition px-4 py-2 text-sm uppercase tracking-wider">
                        Reset Filters
                    </button>
                    <button wire:click="applyFilters" class="px-8 py-3 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 shadow-lg shadow-red-200 transition transform hover:scale-[1.02] active:scale-95">
                        Show Results
                    </button>
                </div>
            </div>
