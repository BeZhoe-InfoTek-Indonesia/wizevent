            ...
            <div class="p-6 md:p-8 space-y-6">
                ...
                <div class="grid grid-cols-2 gap-y-6 gap-x-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm md:text-base font-black text-gray-900">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</p>
                    </div>
                    <div>
                ...
                {{-- Upload Payment Proof or Chat Buttons --}}
                <div class="space-y-4">
                    @if(!$order->paymentProof)
                        <div class="space-y-2">
                            <label class="block text-sm font-bold text-gray-700">{{ __('payment_proof.label') }}</label>
                            <input type="file" wire:model="paymentProof" accept=".jpg,.jpeg,.png,.pdf" class="block w-full text-sm text-gray-700 bg-white rounded-md border border-gray-200 p-2" />
                            @error('paymentProof') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror

                            <div class="flex items-center gap-3">
                                <button wire:click="uploadProof" wire:loading.attr="disabled" wire:target="uploadProof, paymentProof" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-black py-3 rounded-xl transition-shadow">
                                    <span wire:loading.remove wire:target="uploadProof, paymentProof">Upload Payment Proof</span>
                                    <span wire:loading wire:target="uploadProof, paymentProof">Uploading...</span>
                                </button>

                                <a href="https://wa.me/628123456789" target="_blank" class="w-1/3 bg-[#25D366] hover:bg-[#20bd5a] text-white font-black py-3 rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg shadow-green-500/20 active:scale-[0.98]">
                            ...
                            @if($statusMessage)
                                <p class="text-sm text-green-700 font-bold">{{ $statusMessage }}</p>
                            @endif
                        </div>
                    @else
                        {{-- Show uploaded payment proof preview and status --}}
                        <div class="rounded-lg bg-white border p-3">
                            @php $fb = $order->paymentProof->fileBucket; @endphp
                            <div class="flex items-center gap-4">
                                @if(\Illuminate\Support\Str::startsWith($fb->mime_type, 'image'))
                                    <img src="{{ $fb->url }}" alt="{{ $fb->original_filename ?? 'Proof' }}" class="w-20 h-20 object-cover rounded-md" />
                                @else
                                    <div class="w-20 h-20 flex items-center justify-center bg-gray-100 rounded-md">
                                        <svg class="w-8 h-8 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 11c0-1.657-1.343-3-3-3S6 9.343 6 11s1.343 3 3 3 3-1.343 3-3zM18 8v6a2 2 0 01-2 2H8l-4 4V6a2 2 0 012-2h10a2 2 0 012 2z"/></svg>
                                    </div>
                                @endif
                                <div>
                                    <p class="font-bold text-sm">{{ $fb->original_filename ?? __('payment_proof.file') }}</p>
                                    <p class="text-xs text-gray-500">{{ $fb->formatted_size }}</p>
                                    <p class="text-xs mt-1">Status: <span class="font-bold">{{ __('payment_proof.status.' . $order->paymentProof->status) }}</span></p>
                                    <div class="mt-2">
                                        <a href="{{ $fb->url }}" target="_blank" class="text-sm text-blue-600 font-bold">View / Download</a>
                                    </div>
                                </div>
                            </div>
                        </div>
