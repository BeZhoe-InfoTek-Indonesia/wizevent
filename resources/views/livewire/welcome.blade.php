                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
                    @foreach($categories as $category)
                        <button 
                            ...
                            class="group bg-white rounded-xl p-6 shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border-2 border-transparent hover:border-primary-500"
                        >
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-12">
                <div>
                    ...
                    <p class="text-gray-600">
                        {{ __('welcome.events_subtitle', ['count' => $events->count()]) }}
                    </p>
                </div>
                ...
                <div class="mt-4 md:mt-0 flex flex-wrap gap-2">
                    <button 
                        ...
                        class="px-4 py-2 rounded-lg text-sm font-medium transition-colors {{ $dateFilter === 'upcoming' ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}"
                    >
                        {{ __('welcome.filter_upcoming') }}
                    </button>
                    <button 
                        wire:click="filterByDate('this_week')"
                        class="px-4 py-2 rounded-lg text-sm font-medium transition-colors {{ $dateFilter === 'this_week' ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}"
                    >
                        {{ __('welcome.filter_this_week') }}
                    </button>
                    <button 
                        wire:click="filterByDate('this_month')"
                        class="px-4 py-2 rounded-lg text-sm font-medium transition-colors {{ $dateFilter === 'this_month' ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}"
                    >
        ...
        <div class="container mx-auto px-4">
            <div class="max-w-4xl mx-auto text-center">
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    @if(auth()->check())
                        <a href="{{ route('events.create') }}" class="px-8 py-4 bg-white text-primary-600 font-semibold rounded-lg hover:bg-gray-100 transition-colors shadow-lg">
                            {{ __('welcome.cta_create_event') }}
                        </a>
                    @else
                        <a href="{{ route('register') }}" class="px-8 py-4 bg-white text-primary-600 font-semibold rounded-lg hover:bg-gray-100 transition-colors shadow-lg">
                            {{ __('welcome.cta_register') }}
                        </a>
                    @endif
                    <a href="{{ route('events.index') }}" class="px-8 py-4 bg-transparent border-2 border-white text-white font-semibold rounded-lg hover:bg-white/10 transition-colors">
                        {{ __('welcome.cta_browse_events') }}
                    </a>
                </div>
            </div>
        </div>
