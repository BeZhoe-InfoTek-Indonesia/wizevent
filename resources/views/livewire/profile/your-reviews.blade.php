<div class="space-y-6">
    ...
    <div class="glass-panel rounded-3xl shadow-float p-6 md:p-8 bg-white/80 dark:bg-gray-800/80 mb-8">
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
                <h2 class="text-2xl md:text-3xl font-black text-gray-900 dark:text-gray-100 tracking-tight">{{ __('profile.your_reviews') ?? 'Your Reviews' }}</h2>
                <div class="flex items-center gap-2 mt-1">
                    <span class="w-2 h-2 bg-red-500 rounded-full shadow-[0_0_8px_rgba(239,68,68,0.4)]"></span>
                    <p class="text-xs font-black text-gray-400 uppercase tracking-[0.2em]">{{ __('profile.your_reviews_desc') ?? 'Manage feedback' }}</p>
                </div>
            </div>
            ...
            <div class="relative group min-w-[240px]">
                ...
                <select 
                    ...
                    class="block w-full pl-12 pr-4 py-4 rounded-2xl border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50 text-gray-900 dark:text-gray-100 shadow-skeuo-inset focus:ring-2 focus:ring-red-500/30 focus:border-red-500 transition-all text-xs font-black uppercase tracking-widest cursor-pointer appearance-none"
                >
                    <option value="latest">{{ __('profile.sort_latest') ?? 'Newest First' }}</option>
                    <option value="oldest">{{ __('profile.sort_oldest') ?? 'Oldest First' }}</option>
                    <option value="rating_high">{{ __('profile.sort_rating_high') ?? 'Top Rated' }}</option>
                    <option value="rating_low">{{ __('profile.sort_rating_low') ?? 'Lowest Rated' }}</option>
                </select>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <!-- Total Reviews -->
        <div class="extruded-block p-6 rounded-3xl flex items-center justify-between group">
            <div>
                <p class="text-xs font-black text-gray-400 uppercase tracking-widest">{{ __('profile.total_reviews') ?? 'Total Reviews' }}</p>
                <p class="text-3xl font-black text-gray-900 dark:text-gray-100 mt-1 tracking-tighter">{{ $this->totalReviewsCount }}</p>
            </div>
        ...
        <div class="extruded-block p-6 rounded-3xl flex items-center justify-between group">
            <div>
                <p class="text-xs font-black text-gray-400 uppercase tracking-widest">{{ __('profile.helpful_feedback') ?? 'Helpful Feedback' }}</p>
                <p class="text-3xl font-black text-gray-900 dark:text-gray-100 mt-1 tracking-tighter">{{ $this->totalHelpfulVotes }}</p>
            </div>
             <div class="w-14 h-14 bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl flex items-center justify-center text-white shadow-lg group-hover:rotate-6 transition-transform">
                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @if($statusFilter === 'to_review')
        @if($this->eventsToReview->count() > 0)
            <div>
                <h3 class="text-lg font-black text-gray-900 dark:text-gray-100 mb-4 pl-1">{{ __('profile.events_to_review') ?? 'Events to Review' }}</h3>
