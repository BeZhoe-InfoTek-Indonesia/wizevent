...
@section('profile-content')
    <div class="profile-content space-y-6">
        ...
        <div class="lg:hidden flex items-center gap-3 mb-4 sticky top-0 bg-white/90 backdrop-blur-md dark:bg-gray-800/90 z-20 -mx-4 px-4 py-3 border-b border-gray-100 dark:border-gray-700 shadow-sm">
            <button @click="showMobileTab = false" class="group inline-flex items-center justify-center w-8 h-8 rounded-full bg-gray-50 dark:bg-gray-700/50 text-gray-500 dark:text-gray-400 hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-900/20 dark:hover:text-red-400 transition-all duration-200">
            ...
            <div>
                <h2 class="text-base font-bold text-gray-900 dark:text-gray-100" x-text="activeTab === 'dashboard' ? '{{ __('profile.dashboard') }}' : activeTab === 'account' ? '{{ __('profile.account_settings') }}' : activeTab === 'reviews' ? '{{ __('profile.your_reviews') }}' : activeTab === 'wishlist' ? '{{ __('profile.wishlist') }}' : activeTab === 'orders' ? '{{ __('profile.your_orders') }}' : activeTab === 'help' ? '{{ __('profile.help_center') }}' : activeTab === 'settings' ? '{{ __('profile.settings') }}' : '{{ __('profile.dashboard') }}'"></h2>
            </div>
        </div>
