    ...
    <div class="flex flex-col lg:flex-row gap-6">
        ...
        <aside class="lg:w-64 flex-shrink-0">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 mb-6 text-center border border-gray-100 dark:border-gray-700">
                <div class="relative inline-block mb-4">
                    @if($order->user && $order->user->avatar)
                        <img class="h-24 w-24 rounded-full object-cover mx-auto border-4 border-white dark:border-gray-700 shadow-md" src="{{ Storage::url($order->user->avatar) }}" alt="{{ $order->user->name }}">
                    @else
                        <div class="h-24 w-24 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center mx-auto border-4 border-white dark:border-gray-700 shadow-md">
                            <span class="text-3xl text-white font-bold">{{ strtoupper(substr($order->user->name ?? '', 0, 2)) }}</span>
                        </div>
                    @endif
                ...
                <h3 class="font-bold text-gray-900 dark:text-gray-100 text-lg">{{ $order->user?->name }}</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">{{ __('profile.verified_member') }}</p>
                <a href="{{ route('profile', ['tab' => 'account']) }}" class="inline-flex items-center justify-center px-4 py-2 bg-red-50 text-red-600 border border-red-200 rounded-full text-sm font-medium hover:bg-red-100 transition-colors">
            ...
            <nav role="tablist" aria-orientation="vertical" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden border border-gray-100 dark:border-gray-700">
                <a href="{{ route('profile', ['tab' => 'dashboard']) }}" role="tab" class="flex items-center gap-3 px-4 py-3">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/>
                    </svg>
                    <span>{{ __('profile.dashboard') }}</span>
                </a>
                <a href="{{ route('profile', ['tab' => 'account']) }}" role="tab" class="flex items-center gap-3 px-4 py-3 border-t border-gray-100 dark:border-gray-700">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                    </svg>
                    <span>{{ __('profile.account') }}</span>
                </a>
                <a href="{{ route('profile', ['tab' => 'reviews']) }}" role="tab" class="flex items-center gap-3 px-4 py-3 border-t border-gray-100 dark:border-gray-700">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>
                    </svg>
                    <span>{{ __('profile.your_reviews') }}</span>
                </a>
                <a href="{{ route('profile', ['tab' => 'wishlist']) }}" role="tab" class="flex items-center gap-3 px-4 py-3 border-t border-gray-100 dark:border-gray-700">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>
                    </svg>
                    <span>{{ __('profile.wishlist') }}</span>
                </a>
                <a href="{{ route('profile', ['tab' => 'orders']) }}" role="tab" class="flex items-center gap-3 px-4 py-3 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/>
                    </svg>
                    <span>{{ __('profile.your_orders') }}</span>
                </a>
                <a href="{{ route('profile', ['tab' => 'help']) }}" role="tab" class="flex items-center gap-3 px-4 py-3 border-t border-gray-100 dark:border-gray-700">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('profile', ['tab' => 'settings']) }}" role="tab" class="flex items-center gap-3 px-4 py-3 border-t border-gray-100 dark:border-gray-700">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="space-y-8">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
                            <div class="flex items-center gap-5">
                                <div>
                                    <h1 class="text-4xl font-black text-gray-900 dark:text-gray-100 tracking-tight">Order Details</h1>
                                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">#{{ is_array($order->order_number ?? null) ? json_encode($order->order_number) : ($order->order_number ?? '') }} · Placed on {{ $order->created_at->format('M d, Y') }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
                            <div class="xl:col-span-2 space-y-8">
                                ...
                                <div class="bg-white dark:bg-gray-800 rounded-3xl p-6 sm:p-8 border border-orange-100 dark:border-orange-500/20 shadow-sm relative overflow-hidden">
                                    ...
                                    <div class="relative flex flex-row items-start gap-6">
                                        ...
                                        <div>
                                            <h4 class="text-lg font-black text-gray-900 dark:text-white tracking-tight">Payment Verified</h4>
                                            <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mt-2 leading-relaxed max-w-2xl">
                                                You're all set! We've generated your secure tickets below. Please have the QR code ready to scan at the venue entrance.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-6" x-data="{ expandedTicket: {{ $order->tickets->first()?->id ?? 'null' }} }">
                                            ...
                                            <div x-show="expandedTicket === {{ $ticket->id }}"
                                                ...
                                                <div class="flex flex-col lg:flex-row">
                                                    ...
                                                    <div class="p-6 sm:p-8 flex flex-col items-center justify-center bg-gray-50/50 dark:bg-gray-800/50 lg:w-72 lg:border-r border-dashed border-gray-200 dark:border-gray-700 relative overflow-hidden">
                                                        ...
                                                        <p class="mt-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">Scan at Entrance</p>
                                                    ...
                                                    <div class="flex-1 p-6 sm:p-8 relative">
                                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8">
                                                            <div>
                                                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Event</p>
                                                            ...
                                                            <div>
                                                                ...
                                                                <p class="text-sm font-bold text-gray-900 dark:text-white">
                                                                    {{ $order->event->event_date->format('M d, Y') }} at {{ $order->event->event_date->format('H:i') }}
                                                                </p>
                                                            </div>

                                                            <div>
                                                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Venue</p>
                                                                <p class="text-sm font-bold text-gray-900 dark:text-white">
                                                            ...
                                                            <div>
                                                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Pass Type</p>
                                                        ...
                                                        <div class="mt-8 pt-6 border-t border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                                                            ...
                                                            <div class="hidden sm:block text-[10px] text-gray-400 ml-auto font-medium">
                                                                ORDER #{{ is_array($order->order_number ?? null) ? json_encode($order->order_number) : ($order->order_number ?? '') }}
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                    ...
                                    @empty
                                        <div class="bg-white dark:bg-gray-800 rounded-2xl p-8 border-2 border-dashed border-gray-200 dark:border-gray-700 text-center">
                                            ...
                                            <h3 class="text-sm font-bold text-gray-900 dark:text-white">No Tickets Found</h3>
                                            <p class="text-xs text-gray-500 mt-1">There do not seem to be any tickets associated with this order.</p>
                                        </div>
                                    @endforelse
                                </div>
                                ...
                                <div class="bg-white dark:bg-gray-800 rounded-3xl p-6 sm:p-8 shadow-sm border border-gray-200 dark:border-gray-700">
                                    <div class="flex items-center gap-4 mb-8">
                                        ...
                                        <h3 class="text-xl font-black text-gray-900 dark:text-white tracking-tight">Payment Summary</h3>
                                    </div>
                                    ...
                                    <div class="space-y-4">
                                        @foreach($order->orderItems as $item)
                                            <div class="flex justify-between items-center p-4 rounded-2xl bg-gray-50 dark:bg-gray-700/30">
                                                <div>
                                                    ...
                                                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 mt-0.5">Quantity: {{ $item->quantity }}x</p>
                                                </div>
                                        ...
                                        <div class="pt-6 mt-6 border-t border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-end gap-2">
                                            <div class="text-left w-full sm:w-auto">
                                                <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Total Paid</p>
                                                <p class="text-[10px] text-gray-400">All taxes included</p>
                                            </div>
                            ...
                            <div class="space-y-8">
                                <div class="bg-white dark:bg-gray-800 rounded-3xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
                                    <h3 class="text-sm font-black text-gray-900 dark:text-white mb-6 uppercase tracking-wider">Quick Actions</h3>
                                    <div class="space-y-3">
                ...
                @else
                    <div class="space-y-8">
                            ...
                            <div class="bg-gray-900 rounded-[2rem] p-8 text-white relative overflow-hidden shadow-2xl">
                                ...
                                <div class="relative z-10 flex flex-col md:flex-row items-center gap-8">
                                    ...
                                    <div class="flex-1 text-center md:text-left">
                                        <h3 class="text-2xl font-black tracking-tight">Verifying Payment</h3>
                                        <p class="text-gray-400 text-sm mt-2 leading-relaxed max-w-xl">We've received your proof. Our team is now cross-checking the transaction. Hang tight, this usually takes just a few minutes!</p>
                                    </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-[2rem] p-10 shadow-sm border border-gray-100 relative overflow-hidden">
                            ...
                            <h3 class="text-xl font-black text-gray-900 dark:text-gray-100 mb-2 tracking-tight">Payment Proof</h3>
                            <p class="text-sm text-gray-500 dark:text-gray-400 mb-10">Please upload a screenshot of your transaction to speed up verification.</p>
                            ...
                            <div x-data="{ dragging: false }" @dragover.prevent="dragging = true" @dragleave.prevent="dragging = false" @drop.prevent="dragging = false; $refs.fileInput.files = $event.dataTransfer.files; $refs.fileInput.dispatchEvent(new Event('change'))" class="border-2 border-dashed rounded-3xl p-12 text-center transition-all relative group" :class="{ 'border-red-500 bg-red-50': dragging, 'border-gray-100 bg-gray-50/50 hover:bg-gray-50 hover:border-gray-200': !dragging }">
                                ...
                                <div wire:loading.remove wire:target="paymentProof">
                                    ...
                                    <p class="text-gray-900 font-black tracking-tight mb-1 text-lg">Upload Screenshot</p>
                                    <p class="text-sm text-gray-500 mb-6">Drag and drop or <button type="button" @click="$refs.fileInput.click()" class="text-red-600 font-bold hover:underline">browse files</button></p>
                                ...
                                <div wire:loading wire:target="paymentProof" class="absolute inset-0 flex flex-col items-center justify-center bg-white/90 backdrop-blur-sm rounded-3xl z-20">
                                    ...
                                    <p class="text-lg font-black text-gray-900 tracking-tight">Uploading...</p>
                                </div>
                            </div>
                            ...
                            @isset($paymentProof)
                                <div class="mt-8 bg-gray-900 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-xl">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p class="text-white font-bold text-sm truncate max-w-[200px]">{{ method_exists($paymentProof, 'getClientOriginalName') ? $paymentProof->getClientOriginalName() : (is_string($paymentProof) ? $paymentProof : 'Payment Proof') }}</p>
                                            <p class="text-gray-400 text-[10px] uppercase font-bold tracking-widest mt-0.5">Ready to submit</p>
                                        </div>
                                    </div>
                                    <button wire:click="uploadPaymentProof" class="w-full sm:w-auto px-8 py-4 bg-white text-gray-900 text-[10px] font-black rounded-xl uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all shadow-lg active:scale-95">Confirm Submission</button>
                                </div>
                            @endisset

                            @if($order->files->where('bucket_type', 'payment-proofs')->isNotEmpty())
                                <div class="mt-10">
                                    <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-6">Uploaded Documents</h4>
                                        ...
                                        @foreach($order->files->where('bucket_type', 'payment-proofs') as $file)
                                            <div class="relative group aspect-square rounded-2xl overflow-hidden bg-gray-50 border border-gray-100 shadow-sm">
                                                ...
                                                <div class="absolute inset-0 bg-gray-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                                                    <a href="{{ $file->url }}" target="_blank" class="px-4 py-2 bg-white text-gray-900 text-[10px] font-black rounded-lg uppercase tracking-wider">Preview</a>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                            ...
                            @endif
                        <div class="bg-white dark:bg-gray-800 rounded-3xl p-6 sm:p-8 shadow-sm border border-gray-200 dark:border-gray-700">
                            <h3 class="text-xl font-black text-gray-900 dark:text-white mb-8 tracking-tight">Order Information</h3>
