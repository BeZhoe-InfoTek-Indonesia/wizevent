...
#### Scenario: Replacing Monolithic Form
- The existing `form()` method in `EventResource`
- Should be replaced entirely with `Wizard::make([...])` structure
- Removing the old `Group::make()` and `Section::make()` root layout
