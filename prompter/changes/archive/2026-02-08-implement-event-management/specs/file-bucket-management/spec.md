# Capability: File Bucket Management

## Overview
Polymorphic file storage system providing centralized file management for all uploads across the application. Supports multiple file types, automatic image processing, and organized storage with metadata tracking.
...
- Event banners use bucket_type "event-banners"
- Event galleries use bucket_type "event-galleries"
- Events can retrieve their files via polymorphic relationship

### With Event Categorization
- Category icons use bucket_type "category-icons"
- Categories can retrieve their icon via polymorphic relationship

### With Ticket Type Management (Future)
- Ticket type images use bucket_type "ticket-type-images"
- Ticket types can have promotional images

### With Settings Component
- Settings can reference file bucket types for configuration
- File bucket types can be managed via settings
