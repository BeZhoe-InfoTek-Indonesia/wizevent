...
## Overview
Core event CRUD operations and lifecycle management including draft, publish, sold out, and cancelled states.
...
Events can be assigned to categories and tagged for organization.

#### Scenario: Assign category to event
**Given** an authenticated user with `events.edit` permission  
**When** they assign a category to an event  
**Then** the category relationship is established  
**And** the event can be filtered by category

#### Scenario: Assign tags to event
**Given** an authenticated user with `events.edit` permission  
**When** they assign multiple tags to an event  
**Then** the tag relationships are established  
**And** the event can be filtered by tags
